% Read in audio data
audio_file_path = 'Twinkle_Twinkle_Edited.mp3';
...
minPeakDistance = 20; % 7 for Fur Elise
minPeakHeight = 5; %1 for Fur Elise
...
minNegPeakHeight = -30; %-100 for Fur Elise
[~, note_end_indices] = findpeaks(-moving_avg_energy, 'MinPeakDistance', minPeakDistance, 'MinPeakHeight', minNegPeakHeight);

note_start_indices = note_start_indices(1:end); 
note_end_indices = note_end_indices(2:end-1);
...
if length(note_start_indices) == length(note_end_indices)
    ...
    for i=1:length(data_note_start_indices)
...
duration = 0.5; % 1 second per frequency
...
audiowrite('Twinkle_Twinkle_Edited.mp3', reconstr_audio, sample_rate)
