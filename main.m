...
function outSig=bpfProcessing(signal, freqs, Fs)

    outSig=zeros(1, length(signal));
    for lf=freqs
        outSig = outSig+filter(BPF(lf,Fs), signal)';
    end

end
% 
function Hd = BPF(f,Fs)
    % MATLAB Code
    % Generated by MATLAB(R) 23.2 and DSP System Toolbox 23.2.
    % Generated on: 30-Jan-2024 19:50:58

    % Elliptic Bandpass filter designed using FDESIGN.BANDPASS.

    % All frequency values are in Hz.
    hbw = 3;
    Fpass1 = f-hbw;    % First Passband Frequency
    Fpass2 = f+hbw;    % Second Passband Frequency
    Fstop1 = f-2*hbw;    % First Stopband Frequency
    Fstop2 = f+2*hbw;    % Second Stopband Frequency
...
% % Design a low-pass filter with a cutoff of 1 kHz
% nyquist = Fs / 2;  % Nyquist frequency
% cutoff = 840;     % Cutoff frequency in Hz
% [n, Wn] = buttord(cutoff / nyquist, (cutoff + 200) / nyquist, 3, 20);  % Determine the filter order
% [b, a] = butter(n, Wn);  % Design the Butterworth filter
% 
% % Apply the filter to the sound matrix
% filteredSoundMat = filter(b, a, soundMat);

% Plot filtered sound matrix
% sound(filteredSoundMat, Fs);
